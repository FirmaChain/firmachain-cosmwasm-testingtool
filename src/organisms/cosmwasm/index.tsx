...
const CosmWasm = () => {
    const { isValidAddress, cosmwasmStoreCode, cosmwasmInstantiateContract, cosmwasmExecuteContract, cosmwasmMigrateContract } = useFirma()
    ...
    useEffect(() => {
        setActiveStoreCode(
            walletState.mnemonic !== '' && storeWasmBinary.length > 0 && (store.accessConfig === 1 ? isValidAddress(store.address) : true)
        )
    ...
    const txInstantiateContract = (resolveTx: () => void, rejectTx: () => void) => {
        cosmwasmInstantiateContract(walletState.mnemonic, instantiate.codeId, instantiate.funds, instantiate.label, instantiate.args)
